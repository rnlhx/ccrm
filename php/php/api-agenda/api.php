<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        if(isset($_GET['id'])){
            ...
            $id = $_GET['id'];
            $sql = $con->query("select * from agenda_pastoral where id='$id'");
            $data = $sql->fetch_assoc();
        }else{
            // Entra nesse, caso não tenha passagem de ID via "get"
            $data = array();
            $sql = $con->query("select * from agenda_pastoral where data between NOW() and DATE_ADD(NOW(), INTERVAL 24 MONTH)ORDER BY data");
    ...
    if($_SERVER['REQUEST_METHOD'] === 'PUT'){
        ...
        if(isset($_GET['id'])){
            ...
            $sql = $con->query("update agenda_pastoral set  
                data = '".$data->data."',
                time = '".$data->time."', 
                tarefa = '".$data->tarefa."' 
                where id = '$id'");
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        // gravar informações
        $data = json_decode(file_get_contents("php://input"));
        $sql = $con->query("insert into agenda_pastoral(data, time, tarefa) values ('".$data->data."', '".$data->time."', '".$data->tarefa."')");
        if($sql){
    ...
    if($_SERVER['REQUEST_METHOD'] === 'DELETE'){
        ...
        if(isset($_GET['id'])){
            $id = $_GET['id'];
            $sql = $con->query("delete from agenda_pastoral where id='$id'");
            if($sql){
                exit(json_encode(array('status' => 'Sucesso')));
            }else{
